...
function exportPdf(view) {
  const ts = new Date();
  const dateStr = ts.toLocaleDateString('ru-RU', { year:'numeric', month:'long', day:'numeric' });
  const timeStr = ts.toLocaleTimeString('ru-RU', { hour:'2-digit', minute:'2-digit' });

  // Insert print-only header
  const existingHeader = document.querySelector('.pdf-header');
  if (existingHeader) existingHeader.remove();
  const existingMeta = document.querySelector('.pdf-meta');
  if (existingMeta) existingMeta.remove();

  const header = document.createElement('div');
  header.className = 'pdf-header';
  header.textContent = '◆ CX Profit  —  ' + VIEW_NAMES[view];
  document.querySelector('.view').prepend(header);

  const meta = document.createElement('div');
  meta.className = 'pdf-meta';
  meta.textContent = 'Отчёт сгенерирован: ' + dateStr + ' в ' + timeStr + '  |  Данные являются конфиденциальными';
  document.querySelector('.view').prepend(meta);
...
function addPdfButton() {
  const existing = document.querySelector('.pdf-btn');
  if (existing) existing.remove();
  const btn = document.createElement('button');
  btn.className = 'pdf-btn';
  btn.innerHTML = '📄 Скачать PDF';
  btn.onclick = () => exportPdf(currentView);
